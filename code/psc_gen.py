...
def pipeline(n, d, k, reference, a_q, GEN_TIME=300, ILP_TIME=900,
    ...
    if PREVIOUS_POOL:
        ...
        for path in paths:
            if not os.path.exists(path):
                print(f"  generator 0: {path} does not exist, skipped"); continue
            ...
            for H in data:
                ...
                if all(prod(a, b) in Hf for a in Hf for b in Hf):
                    ...
                    bad += 1
            print(f"  generator 0 ({path}): {good} valid, +{len(pool)-n_before_r} new"
                  + (f", {bad} discarded" if bad else ""))

    # --- generator 1: explicit linear codewords ---
    t0 = time.time()
    lin = linear_codewords(G, k, cap=LIN_CAP, seed=seed)
    n_before = len(pool)
    pool |= set(H for H in lin if G['nondeg'](H))
    print(f"  generator 1 (explicit linear codewords): +{len(pool)-n_before} ({time.time()-t0:.0f}s)")
    ...
    while time.time()-t0 < GEN_TIME*0.85:
        r = random.random()
        # mixture reweighted towards the most productive generators: in U_10 the
        ...
        # walk and the structured generator, so as not to lose the types only they
        # reach.
        if r < 0.55:
    ...
    print(f"  generators 2-5 (lattice/involution/mixed/structured): +{len(pool)-n0} "
          f"({time.time()-t0:.0f}s)")
