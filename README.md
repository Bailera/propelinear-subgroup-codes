...
code/
  ...
  exact_k2.py            exhaustive enumeration of the subgroups of order 4,
                         with a clique found by ILP over the complete list
  exact_highs.py         certified upper bound: linear relaxation over a complete
                         pool, bounded by weak duality from an explicit dual point
...
Exact values for $k=2$ are established by matching bounds over an exhaustive
enumeration. Every subgroup of order $4$ is either cyclic or elementary
abelian, hence generated by one element of order $4$ or by two commuting
involutions, so all of them can be listed; `exact_k2.py` does this, writes the
complete list to `pool_<ndk>_full.pkl`, and finds a clique by ILP, which gives
the lower bound. The upper bound comes from the linear relaxation over the
complete list:

```
python code/exact_k2.py    --n 7 --d 4 --aq 41
python code/exact_highs.py --pool pool_742_full.pkl --n 7 --d 4 --k 2 --aq 41 --lp-only
```

`exact_highs.py` solves the relaxation with the interior-point method of HiGHS
and certifies it independently of the solver's tolerances: from the row duals
$y$ it builds the explicitly dual-feasible pair $(y,z)$ with $y\ge 0$ and
...
before the time limit. The exact values of the paper do not rest on these
flags: each rests on a clique checked by `verify_psc.py` and a matching upper
...
|$(8,4,2)$|$85$|$15060$|exact|`clique_842.json`|
...
Python 3.9 or later and PuLP, which ships the CBC solver; `exact_highs.py`
additionally needs NumPy, SciPy and highspy:
